...
function loadNextReservations(jsonObj)
{
	var div = document.getElementById("nextReservationsFrame");
	var tbody = div.querySelector("tbody");
	
	while (tbody.firstChild) tbody.removeChild(tbody.firstChild);
	
	if (jsonObj.reservations && jsonObj.reservations.length > 0)
	{
		div.style.display = "block";
		
		for (let res of jsonObj.reservations)
		{
			let tr = (function()
			{
				let tdUserId = document.createElement("td");
				tdUserId.innerText = res.userId;
				
				let tdUserName = document.createElement("td");
				tdUserName.innerText = res.userName;
				
				let tdReservationDatetime = document.createElement("td");
				tdReservationDatetime.innerText = res.reservationDatetime;
				
				let tdSelectLink = document.createElement("td");
				tdSelectLink.className = "shrinkCell";
				let selectLink = document.createElement("a");
				selectLink.href = "#";
				selectLink.onclick = linkNextReservationsSelect_onClick;
				selectLink.innerText = "Selecionar";
				tdSelectLink.appendChild(selectLink);
				
				let tr = document.createElement("tr");
				tr.setAttribute("data-userId", res.userId);
				
				tr.appendChild(tdUserId);
				tr.appendChild(tdUserName);
				tr.appendChild(tdReservationDatetime);
				tr.appendChild(tdSelectLink);
...
function linkNextReservationsSelect_onClick(e)
{
	e.preventDefault();
	var userId = this.parentNode.parentNode.getAttribute("data-userId");
	setUserIdInput(userId);
}

function setPublicationIdInput(id)
{
	document.getElementById("frmCreateLoan").elements["numPubId"].value = id;
	btnLoadPublication_onClick();
}

function setUserIdInput(id)
{
	document.getElementById("frmCreateLoan").elements["numUserId"].value = id;
	btnLoadUser_onClick();
}
...
{
	var id = document.getElementById("frmCreateLoan").elements["numPubId"].value;
	
	if (id)
	{
		fetch(fileBasePath + "/generate/libLoanGetSinglePublication.php?id=" + id).then( res => res.json() ).then( json => loadPublication(json) );
		fetch(fileBasePath + "/generate/libResGetNextReservations.php?pubId=" + id).then( res => res.json() ).then( json => loadNextReservations(json) );
	}
}

function btnLoadUser_onClick(e)
{
	var id = document.getElementById("frmCreateLoan").elements["numUserId"].value;
	if (id)
